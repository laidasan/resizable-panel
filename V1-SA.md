...
**iframe 指標釋放偵測**：使用者在拖曳中將指標移入 `<iframe>` 並放開時，pointerup 不會冒泡到外層 document。透過 pointermove 中檢查 `event.buttons === 0` 偵測此情況，強制走結束流程（重置 DragState、reset cursor、觸發 LayoutChanged）。
...
flowchart TD
    ...
    ResetCursor --> EmitChanged["觸發 LayoutChanged 事件<br/>傳送最終 LayoutResult"]
