...
### 方案：改為原版 react-resizable-panels 的 validateLayout 邏輯

將 `_applyConstraints`（validateLayout 路徑）改為原版 `validatePanelGroupLayout` 的策略：

1. **先 normalize** — 加總 ≠ 100% 時，等比例縮放回 100%
2. **clamp** — 每個 panel 依序 clamp 到 min/max，累積 remainingSize（maxSize 永遠勝出）
3. **重分配** — 從 index 0 開始，把 remainingSize 塞回去（respect min/max）

### 移除項目

- `_shrinkPanels` — 從後往前扣 + 第二輪強制突破 minSize
- `_growPanels` — 從後往前補
- `_redistributeOverflow` — 上述兩者的入口

### 取捨

| 得到 | 放棄 |
|------|------|
| 兩條路徑（validateLayout / adjustLayoutByDelta）約束處理一致 — 拖曳跳變自然消失 | 100% 不變式 — 極端約束衝突時 layout 加總可能 > 100% |
| resize 行為更平滑（先等比縮放再微調） | |

### 後續待討論

- panel 溢出容器（加總 > 100%）的合理性與處理策略
